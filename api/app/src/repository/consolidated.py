...
from model.consolidated import Consolidated
...
class ConsolidatedRepository:
    ...
    async def get_all(
        ...
        query = select(from_obj=Consolidated, columns="*")

        # select columns dynamically
        if columns is not None and columns != "all":
            # we need column format data like this --> [column(id),column(name),column(sex)...]

            query = select(from_obj=Consolidated, columns=convert_columns(columns))
        ...
        if filter is not None and filter != "null":
            ...
            for attr, value in criteria.items():
                _attr = getattr(Consolidated, attr)
