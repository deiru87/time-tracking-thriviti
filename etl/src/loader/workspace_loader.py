...
logger = log.get_logger("workspace-loader-component")


class WorkspaceLoaderHandler(AbstractHandler):

    @staticmethod
    def loader(workspaces: DataFrame) -> None:
        logger.info('saving workspace data in database')
        ...
        try:
            ...
            with engine.connect() as conn:
                workspaces.to_sql(con=conn, index=False, name='workspace', if_exists='replace')
        except Exception as error:
            logger.error(error)

    def handle(self, request: Any) -> None:
        self.loader(request)
        logger.info("finish persistence of workspace in DB")
