...
logger = log.get_logger("membership-loader-component")


class MembershipLoaderHandler(AbstractHandler):

    @staticmethod
    def loader(workspaces: DataFrame) -> None:
        logger.info('saving Memberships data in database')
        ...
        try:
            ...
            with engine.connect() as conn:
                workspaces.to_sql(con=conn, index=False, name='membership', if_exists='replace')
        except Exception as error:
            logger.error(error)

    def handle(self, request: Any) -> None:
        self.loader(request)
        logger.info("finish persistence of memberships in DB")
