...
logger = log.get_logger("user-loader-component")


class ProjectLoaderHandler(AbstractHandler):

    @staticmethod
    def loader(workspaces: DataFrame) -> None:
        logger.info('saving Projects data in database')
        ...
        try:
            ...
            with engine.connect() as conn:
                workspaces.to_sql(con=conn, index=False, name='project', if_exists='replace')
        except Exception as error:
            logger.error(error)

    def handle(self, request: Any) -> None:
        self.loader(request)
        logger.info("finish persistence of projects in DB")
